...
def get_historical_policy(agent_id: str, commit_sha: str):
    return BaseAPIResponse(success=True, message="Success", data=version_service.get_historical_policy(agent_id, commit_sha))

@router.post("/rollback", response_model=BaseAPIResponse[dict])
def rollback_policy(agent_id: str, request: PolicyRollbackRequest):
    return BaseAPIResponse(success=True, message="Success", data=version_service.rollback_policy(agent_id, request.commit_sha))
