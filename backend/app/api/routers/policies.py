...
from api.models.requests import PolicyDeployRequest, PolicyDiffRequest, PolicyValidateRequest
...
def validate_policy(request: PolicyValidateRequest):
    return BaseAPIResponse(
        success=True, message="Success", data=policy_service.validate_policy_content(request.policy_yaml_content)
    )
...
def deploy_policy(request: PolicyDeployRequest):
    return BaseAPIResponse(
        success=True,
        message="Success",
        data=policy_service.deploy_policy(request.agent_id, request.policy_yaml_content, request.commit_sha),
    )
...
def diff_policies(request: PolicyDiffRequest):
    return BaseAPIResponse(
        success=True, message="Success", data=policy_service.diff_policies(request.agent_id, request.commit_sha)
    )
