import json
from common.db import run_write, run_query
...
class AuditRepository:
    ...
    def get_recent(limit: int = 50) -> list:
        ...
        return [dict(r) for r in rows][::-1]  # Return in chronological order if desired, or as is. Usually recent is desc, but let's just return what we got.

class ReportRepository:
    @staticmethod
    def save_report_content(account_id: int, content: str, summary: str = "", generated_by: str = "", metadata: dict = None):
        run_write(
...
class HITLRepository:
    ...
    def update_status(request_id: int, status: str, approved_by: str = ""):
        run_write(
            """UPDATE hitl_requests SET status = ?, approved_by = ?, approval_time = datetime('now') WHERE request_id = ?""",
...
class GovernanceVersionRepository:
    ...
    def get_by_commit(commit_sha: str) -> dict:
        rows = run_query("SELECT * FROM governance_versions WHERE git_commit_sha = ? ORDER BY version_number DESC LIMIT 1", (commit_sha,))
        return dict(rows[0]) if rows else None
    ...
    def set_active_version(version_number: int, rolled_back_from: int = None):
        run_write("UPDATE governance_versions SET is_active = 0", ())
        if rolled_back_from:
            run_write("UPDATE governance_versions SET is_active = 1, rolled_back_from = ?, rollback_timestamp = CURRENT_TIMESTAMP WHERE version_number = ?", (rolled_back_from, version_number))
        else:
