...
def create_version(agent_id: str, commit_sha: str, policy_yaml: str) -> dict:
    ...
    rows = run_query(
        "SELECT version_id FROM policy_versions WHERE agent_id = ? AND commit_sha = ? ORDER BY version_id DESC LIMIT 1",
        (agent_id, commit_sha),
    )
    return {
...
def get_versions(agent_id: str) -> list:
    rows = run_query(
        "SELECT version_id, commit_sha, policy_hash, deployed_at, is_active FROM policy_versions WHERE agent_id = ? ORDER BY deployed_at DESC",
        (agent_id,),
    )
...
def _hash_files_in_dir(directory: str | Path, ext: str) -> str:
...
def check_and_create_version_on_startup(change_summary: str = "System startup detected changes in governed files", metadata: dict = None):
    ...
    if active_version:
        if (
            active_version.get("policy_hash") == current_hashes["policy_hash"]
            and active_version.get("agent_hash") == current_hashes["agent_hash"]
            and active_version.get("governance_hash") == current_hashes["governance_hash"]
        ):
            return
...
def rollback_global_version(version_number: int = None, commit_sha: str = None) -> dict:
    ...
    GovernanceVersionRepository.set_active_version(
        target_version["version_number"], rolled_back_from=old_version_number
    )
