...
def list_agents() -> list:
    if not Path(AGENTS_DIR).exists():
        return []
    return [d for d in os.listdir(AGENTS_DIR) if os.path.isdir(AGENTS_DIR / d)]
...
def get_agent_status(agent_id: str) -> dict:
    ...
    return {
        "agent_id": agent_id,
        "readiness": "ready" if Path(yaml_path).exists() else "not_found"
    }
