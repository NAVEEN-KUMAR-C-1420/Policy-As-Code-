...
class AgentPolicyCompatError(Exception):
    ...
    def __init__(self, errors: list):
        self.errors = errors
        super().__init__(
            "Agent/Policy compatibility check failed:\n" +
...
def validate_agent_policy_compat(
    agent_config: dict,
    policy: dict,
    ...
    if agent_id != policy_id:
        errors.append(
            f"Agent ID mismatch: agent.yaml has '{agent_id}' "
            f"but policy.yaml has '{policy_id}'"
        )
    ...
    if requested_model and requested_model not in approved_models:
        errors.append(
            f"Model '{requested_model}' is not in policy approved_models: "
            f"{approved_models}"
        )
    ...
    for tool_name in agent_tools:
        if tool_name not in policy_tools:
            ...
            if not rule.get("allowed", False):
                errors.append(
                    f"Tool '{tool_name}' is declared in agent.yaml but "
                    f"policy.yaml sets allowed=false"
                )
            # Check that the tool's scope is not in denied_scopes
            tool_scope = rule.get("scope", "")
            if tool_scope in denied_scopes:
                errors.append(
                    f"Tool '{tool_name}' has scope '{tool_scope}' which "
                    f"is in denied_scopes: {denied_scopes}"
                )
...
def load_and_validate_agent(agent_dir: str, raise_on_error: bool = True):
    ...
    compat_result = validate_agent_policy_compat(
        agent_config, policy, raise_on_error=raise_on_error
    )
